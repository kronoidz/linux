...
#define PI4IO_DIRECTION_TO_GPIOD(x) ((x) ? GPIO_LINE_DIRECTION_OUT : GPIO_LINE_DIRECTION_IN)
#define GPIOD_DIRECTION_TO_PI4IO(x) ((x) == GPIO_LINE_DIRECTION_OUT ? 1 : 0)
...
{
	return pi4io_gpio_set_direction(chip, offset, GPIO_LINE_DIRECTION_IN);
}
	...
	struct gpio_chip *chip, unsigned offset, int value)
{
	...
	struct pi4io_priv *pi4io = gpiochip_get_data(chip);
	struct device *dev = &pi4io->i2c->dev;
	ret = pi4io_gpio_set_direction(chip, offset, GPIO_LINE_DIRECTION_OUT);
	if (ret) {
		dev_err(dev, "Failed to set direction: %d", ret);
		return ret;
	}
